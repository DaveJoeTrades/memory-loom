...
async function generateSampleStories(n, lang, seedName, onProgress) {
  // One shared "family bible" keeps names, years and places consistent across every story.
  const bible = await generateFamilyBible(lang, seedName);
...
function FamilyView({ graph, mutateGraph, index, setIndexPersist, runExtraction, goStory, storageOk, journal, mutateJournal }) {
  ...
  async function seedGenerated() {
    ...
    try {
      ...
      const gen = await generateSampleStories(seedN, LOOM_LANG, spName,
        (done, total) => setSeedMsg(ft("Claude has written ") + done + "/" + total + ft(" stories…")));
