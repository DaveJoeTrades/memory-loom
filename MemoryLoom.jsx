...
function FamilyView({ graph, mutateGraph, index, setIndexPersist, runExtraction, goStory, storageOk, journal, mutateJournal }) {
  ...
  const [seedPeople, setSeedPeople] = useState(2);
    ...
    for (const f of files) {
      ...
      mutateGraph(g => {
        ...
        g.inbox.push({ id: uid(), forSpeakerId: askFor, fromName: fromName.trim() || ft("Photo album"),
          q, photoId: pid, status: "queued", skips: 0, createdAt: Date.now() });
      });
      n++;
    }
  ...
  const tabs = [
    ...
    ["keepsakes", ft("Things"), Package, (graph.inbox || []).filter(i => i.photoId).length + graph.objects.filter(o => o.photoId).length],
              ...
              <Card style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <span style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    ...
                    <input type="number" min="1" max="6" value={seedPeople}
                      onChange={e => setSeedPeople(Math.max(1, Math.min(6, parseInt(e.target.value || "1", 10))))}
