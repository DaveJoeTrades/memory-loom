...
function TreeGraph({ graph, onPick }) {
  // Generation bands with couples drawn side by side and a marriage bar between them.
  const rootId = graph.settings && graph.settings.rootSpeakerId;
  const rootName = ((graph.settings.speakers || []).find(x => x.id === rootId) || {}).name || "\u2605";
  const bands = [
    ["grandparents", "Grandparents"], ["parents", "Parents"], ["self", "This generation"],
    ["children", "Children"], ["grandchildren", "Grandchildren"], ["extended", "Extended family"]
  ];
  const placed = {};
  for (const p of (graph.people || [])) {
    const gen = genOf(p.rel);
    if (gen === "unplaced") continue;
    const row = (gen === "siblings" || gen === "spouse") ? "self" : gen;
    (placed[row] = placed[row] || []).push(p);
  }
  placed.self = placed.self || [];
  if (!placed.self.some(p => (p.name || "").trim().toLowerCase() === rootName.trim().toLowerCase())) {
    placed.self.unshift({ id: "__root", name: rootName, rel: "\u2605", isRoot: true });
  }
  // Spouses sit next to the person they married.
  const spouseOf = {};
  for (const k of (graph.kin || [])) {
    if (!/spouse|wife|husband|married/i.test(k.rel || "")) continue;
    spouseOf[k.aId] = k.bId; spouseOf[k.bId] = k.aId;
  }
  const used = bands.filter(b => (placed[b[0]] || []).length);
  const NODE_W = 132, NODE_H = 48, GAP_X = 26, ROW_H = 108, PAD_TOP = 26;
  const widest = Math.max(1, ...used.map(b => (placed[b[0]] || []).length));
  const W = Math.max(640, widest * (NODE_W + GAP_X) + 60);
  const H = used.length * ROW_H + PAD_TOP + 12;
  const pos = {};
  used.forEach((b, ri) => {
    const list = (placed[b[0]] || []).slice().sort((x, y) => {
      const ax = spouseOf[x.id] ? 0 : 1, ay = spouseOf[y.id] ? 0 : 1;
      return ax - ay || String(x.name).localeCompare(String(y.name));
    });
    const total = list.length * NODE_W + (list.length - 1) * GAP_X;
    let x = (W - total) / 2 + NODE_W / 2;
    list.forEach(p => { pos[p.id] = { x, y: PAD_TOP + ri * ROW_H + NODE_H / 2, p, row: b[0], ri }; x += NODE_W + GAP_X; });
  });
  const nodes = Object.values(pos);
  return (
    <div style={{ overflowX: "auto", overflowY: "hidden", WebkitOverflowScrolling: "touch" }}>
      <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", minWidth: Math.min(W, 760), height: H, display: "block" }}>
        {used.map((b, ri) => (
          <g key={b[0]}>
            <line x1={0} x2={W} y1={PAD_TOP + ri * ROW_H - 16} y2={PAD_TOP + ri * ROW_H - 16} stroke={T.line} strokeWidth="1" strokeDasharray="3 5" />
            <text x={8} y={PAD_TOP + ri * ROW_H - 21} style={{ fontFamily: T.sans, fontSize: 10, fill: T.faded, letterSpacing: "0.12em", textTransform: "uppercase" }}>{ft(b[1])}</text>
          </g>
        ))}
        {nodes.map(a => {
          const partner = spouseOf[a.p.id] && pos[spouseOf[a.p.id]];
          if (!partner || partner.x < a.x) return null;
          return <line key={"m" + a.p.id} x1={a.x + NODE_W / 2} x2={partner.x - NODE_W / 2} y1={a.y} y2={a.y} stroke={T.brass} strokeWidth="2.5" />;
        })}
        {nodes.map(a => {
          if (a.ri === 0) return null;
          const above = nodes.filter(b => b.ri === a.ri - 1);
          if (!above.length) return null;
          const mid = above.reduce((sum, b) => sum + b.x, 0) / above.length;
          const yTop = a.y - NODE_H / 2, yBot = a.y - ROW_H + NODE_H / 2;
          return <path key={"e" + a.p.id} d={"M " + a.x + " " + yTop + " C " + a.x + " " + (yTop - 28) + ", " + mid + " " + (yBot + 28) + ", " + mid + " " + yBot}
        ...
        {nodes.map(a => (
          <g key={a.p.id} onClick={() => onPick && a.p.id !== "__root" && onPick(a.p)} style={{ cursor: a.p.id === "__root" ? "default" : "pointer" }}>
            <rect x={a.x - NODE_W / 2} y={a.y - NODE_H / 2} width={NODE_W} height={NODE_H} rx={12}
              fill={a.p.isRoot ? T.ledger : T.card} stroke={a.p.isRoot ? T.ledger : T.line} strokeWidth="1.5" />
            <text x={a.x} y={a.y - 4} textAnchor="middle" style={{ fontFamily: T.serif, fontSize: 14, fill: a.p.isRoot ? T.card : T.ink }}>
...
function FamilyView({ graph, mutateGraph, index, setIndexPersist, runExtraction, goStory, storageOk, journal, mutateJournal }) {
  ...
  const seedPeople = Math.max(1, Math.min(6, parseInt(seedPeopleTxt, 10) || 1));
              ...
              <Card style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <span style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    ...
                    <input type="number" min="1" max="6" inputMode="numeric" value={seedPeopleTxt}
