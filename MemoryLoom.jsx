...
async function generateSampleStories(n, lang, seedName) {
  // Written by Claude with the family's own key so the sample can be long and varied
  // without shipping thousands of words inside the app.
  const out = [];
  const batch = 5;
  for (let start = 0; start < n; start += batch) {
    const count = Math.min(batch, n - start);
    const prompt = 'You are helping test a family life-story app. Invent ' + count + ' DIFFERENT spoken-sounding stories told by an elderly person called ' + (seedName || "the storyteller") + '.\n' +
      'This is a consistent, invented family — keep the same relatives, places and dates across stories so a family tree can be built from them. ' +
      'Already told (do not repeat, but stay consistent with): ' + (out.length ? out.map(o => o.q).join(" | ") : "nothing yet") + '\n' +
      'Each story: 180\u2013300 words, first person, plain spoken language with digressions, named relatives, a place, and at least one year. No headings, no markdown.\n' +
      (lang === "zh" ? 'Write the question and story in natural Simplified Chinese.\n' : 'Write in plain English.\n') +
      'Chapters to draw from: beginnings, home, family, kin, love, work, places, traditions, hard-times, witness, joy, wisdom.\n' +
    ...
    try {
      ...
      if (Array.isArray(arr)) for (const x of arr) if (x && x.t && x.q) out.push({ chapter: x.chapter || "open", q: x.q, t: x.t });
    } catch (e) { break; }
  }
  return out;
}
...
function FamilyView({ graph, mutateGraph, index, setIndexPersist, runExtraction, goStory, storageOk, journal, mutateJournal }) {
  ...
  async function seedGenerated() {
    ...
    try {
      ...
      const made = await generateSampleStories(seedN, LOOM_LANG, spName);
      if (!made.length) { setSeedMsg(ft("Nothing came back — check the API key in the key gate.")); return; }
      ...
      mutateGraph(g => { g.stats.stories = (g.stats.stories || 0) + made.length; g.stats.minutes = (g.stats.minutes || 0) + made.length * 2.5; g.demoLoaded = true; });
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        try { await runExtraction(ids[i]); } catch (e) {}
      }
