...
function TreeGraph({ graph, onPick }) {
  ...
  const ego = { id: "__root", name: rootName, rel: "\u2605", isRoot: true };
  if (!byRow.self.some(p => (p.name || "").trim().toLowerCase() === rootName.trim().toLowerCase())) byRow.self.unshift(ego);

  // Couples: spouse links from the ledger, plus the root's own mother and father.
  const spouse = {};
  for (const k of (graph.kin || [])) {
    if (!/spouse|wife|husband|marri/i.test(k.rel || "")) continue;
    spouse[k.aId] = k.bId; spouse[k.bId] = k.aId;
  }
  const mum = (byRow.parents || []).find(p => /\b(mother|mom|mum|mama)\b/i.test(p.rel || ""));
  const dad = (byRow.parents || []).find(p => /\b(father|dad|papa)\b/i.test(p.rel || ""));
  if (mum && dad && !spouse[mum.id]) { spouse[mum.id] = dad.id; spouse[dad.id] = mum.id; }

  // Order each row so couples stand together.
  const ordered = {};
  ...
  used.forEach((r, ri) => {
    ...
    const kids = (ordered[r.key] || []).map(p => pos[p.id]).filter(Boolean);
