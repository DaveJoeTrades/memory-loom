...
function TreeGraph({ graph, onPick }) {
  // Drawn as a standard drop-line genealogy chart:
  //   one generation per row · a horizontal bar between spouses · a vertical drop from the
  //   MIDDLE of that bar to a horizontal sibling bar · a short drop to each child.
  //   Right angles only, never diagonals. Aunts and uncles sit on the parents' row.
  const [zoom, setZoom] = useState(1);
  const rootId = graph.settings && graph.settings.rootSpeakerId;
  const rootName = ((graph.settings.speakers || []).find(x => x.id === rootId) || {}).name || "\u2605";

  const ROWS = [
    { key: "grandparents", label: "Grandparents" },
    { key: "parents", label: "Parents, aunts & uncles" },
    { key: "self", label: "This generation" },
    { key: "children", label: "Children" },
    { key: "grandchildren", label: "Grandchildren" }
  ];
  const rowOf = (rel) => {
    const g2 = genOf(rel);
    if (g2 === "grandparents") return "grandparents";
    if (g2 === "parents") return "parents";
    if (g2 === "siblings" || g2 === "spouse") return "self";
    if (g2 === "children") return "children";
    if (g2 === "grandchildren") return "grandchildren";
    if (g2 === "extended") return /aunt|uncle/i.test(rel || "") ? "parents" : "self";
    return null;
  };
  ...
  for (const p of (graph.people || [])) {
    const r = rowOf(p.rel);
    if (!r) continue;
    (byRow[r] = byRow[r] || []).push(p);
  }
  byRow.self = byRow.self || [];
  const ego = { id: "__root", name: rootName, rel: "\u2605", isRoot: true, blood: true };
  if (!byRow.self.some(p => (p.name || "").trim().toLowerCase() === rootName.trim().toLowerCase())) byRow.self.unshift(ego);

  // One mother, one father. Extra entries are the same person split across spellings —
  // keep the best-evidenced and list the rest instead of drawing three parents.
  const dupes = [];
  const mumPick = pickCanonical(byRow.parents || [], /\b(mother|mom|mum|mama)\b/i);
  const dadPick = pickCanonical(byRow.parents || [], /\b(father|dad|papa)\b/i);
  [mumPick, dadPick].forEach(pk => pk.dupes.forEach(d => dupes.push(d)));
  const dupIds = {};
  dupes.forEach(d => { dupIds[d.id] = 1; });
  for (const r of Object.keys(byRow)) byRow[r] = byRow[r].filter(p => !dupIds[p.id]);

  // Spouses: from the ledger, plus the root's own parents, plus anyone whose relation says
  // they married a named person ("Gerald's wife" attaches to Gerald, not to the parents).
  const spouse = {};
  for (const k of (graph.kin || [])) {
    if (!/spouse|wife|husband|marri/i.test(k.rel || "")) continue;
    if (dupIds[k.aId] || dupIds[k.bId]) continue;
    spouse[k.aId] = k.bId; spouse[k.bId] = k.aId;
  }
  const mum = mumPick.keep, dad = dadPick.keep;
  if (mum && dad && !spouse[mum.id]) { spouse[mum.id] = dad.id; spouse[dad.id] = mum.id; }
  const allPeople = Object.keys(byRow).reduce((acc, r) => acc.concat(byRow[r]), []);
  for (const p of allPeople) {
    if (spouse[p.id] || !isMarriedIn(p.rel)) continue;
    const m = String(p.name || "").match(/^(.+?)'s\s+(wife|husband|spouse|partner)$/i) ||
              String(p.rel || "").match(/^(.+?)'s\s+(wife|husband|spouse|partner)$/i);
    if (!m) continue;
    const partner = allPeople.find(x => x.name && x.name.trim().toLowerCase() === m[1].trim().toLowerCase());
    if (partner) { spouse[p.id] = partner.id; spouse[partner.id] = p.id; }
  }

  // Mark who descends from the couple above and who only married in.
  for (const r of Object.keys(byRow)) {
    byRow[r] = byRow[r].map(p => Object.assign({}, p, { blood: p.isRoot ? true : isBloodOf(p.rel, r) }));
  }

  // Order each row so a spouse stands beside the person they married.
  const ordered = {};
  for (const r of ROWS.map(x => x.key)) {
    const list = (byRow[r] || []).slice();
    const bloodFirst = list.filter(p => p.blood).concat(list.filter(p => !p.blood && !list.some(q => spouse[p.id] === q.id)));
    const seen = {}, out = [];
    for (const p of bloodFirst) {
      if (seen[p.id]) continue;
      seen[p.id] = 1; out.push(p);
      const sp = spouse[p.id] && list.find(x => x.id === spouse[p.id]);
      if (sp && !seen[sp.id]) { seen[sp.id] = 1; out.push(sp); }
    }
    for (const p of list) if (!seen[p.id]) { seen[p.id] = 1; out.push(p); }
    ordered[r] = out;
  }
  const used = ROWS.filter(r => (ordered[r.key] || []).length);

  const NW = 128, NH = 46, GAP = 30, ROW_H = 128, PAD = 34;
  const widest = Math.max(1, ...used.map(r => (ordered[r.key] || []).length));
  const W = Math.max(680, widest * (NW + GAP) + 80);
  const H = used.length * ROW_H + PAD + 20;

  const pos = {};
  used.forEach((r, ri) => {
    const list = ordered[r.key];
    const total = list.length * NW + (list.length - 1) * GAP;
    let x = (W - total) / 2 + NW / 2;
    list.forEach(p => { pos[p.id] = { x, y: PAD + ri * ROW_H, p, ri }; x += NW + GAP; });
  });
  const nodes = Object.values(pos);

  // A parent unit is a couple (or a lone parent). Children of the row below hang from it.
  const units = [];
  used.forEach((r, ri) => {
    if (ri >= used.length - 1) return;
    const list = ordered[r.key], taken = {};
    for (const p of list) {
      if (taken[p.id]) continue;
      const sp = spouse[p.id] && list.find(x => x.id === spouse[p.id]);
      if (sp) { taken[p.id] = taken[sp.id] = 1; units.push({ ri, a: pos[p.id], b: pos[sp.id] }); }
      else { taken[p.id] = 1; units.push({ ri, a: pos[p.id], b: null }); }
    }
  });

  // Each row's children attach to the FIRST unit on the row above (the couple line), which is
  // where a real chart puts them when parentage is not individually recorded.
  const drops = [];
  used.forEach((r, ri) => {
    if (ri === 0) return;
    const parentUnits = units.filter(u => u.ri === ri - 1);
    if (!parentUnits.length) return;
    const unit = parentUnits[0];
    const px = unit.b ? (unit.a.x + unit.b.x) / 2 : unit.a.x;
    const py = unit.a.y + NH / 2;
    const kids = (ordered[r.key] || []).filter(p => p.blood).map(p => pos[p.id]).filter(Boolean);
    if (!kids.length) return;
    const barY = py + (ROW_H - NH) / 2;
    const left = Math.min(...kids.map(k => k.x)), right = Math.max(...kids.map(k => k.x));
    drops.push({ px, py, barY, left, right, kids });
  });
    ...
    <div>
      ...
      <div style={{ overflow: "auto", WebkitOverflowScrolling: "touch", border: `1px solid ${T.line}`, borderRadius: 12, background: T.paper, maxHeight: "72vh" }}>
        <svg viewBox={"0 0 " + W + " " + H} style={{ width: W * zoom, height: H * zoom, display: "block" }}>
          {used.map((r, ri) => (
            <text key={r.key} x={10} y={PAD + ri * ROW_H - NH / 2 - 10}
              style={{ fontFamily: T.sans, fontSize: 10, fill: T.faded, letterSpacing: "0.12em", textTransform: "uppercase" }}>{ft(r.label)}</text>
          ))}
          {units.filter(u => u.b).map((u, n) => (
            <g key={"u" + n}>
              <line x1={u.a.x + NW / 2} x2={u.b.x - NW / 2} y1={u.a.y} y2={u.a.y} stroke={T.brass} strokeWidth="2.5" />
              <line x1={(u.a.x + u.b.x) / 2 - 5} x2={(u.a.x + u.b.x) / 2 + 5} y1={u.a.y - 4} y2={u.a.y - 4} stroke={T.brass} strokeWidth="2" />
              <line x1={(u.a.x + u.b.x) / 2 - 5} x2={(u.a.x + u.b.x) / 2 + 5} y1={u.a.y + 4} y2={u.a.y + 4} stroke={T.brass} strokeWidth="2" />
            </g>
          ))}
          {drops.map((d, n) => (
            <g key={"d" + n}>
              ...
              {d.kids.map(k => <line key={k.p.id} x1={k.x} x2={k.x} y1={d.barY} y2={k.y - NH / 2} stroke={T.line} strokeWidth="1.8" />)}
            </g>
          ))}
          {nodes.map(a => (
            <g key={a.p.id} onClick={() => onPick && !a.p.isRoot && onPick(a.p)} style={{ cursor: a.p.isRoot ? "default" : "pointer" }}>
              <rect x={a.x - NW / 2} y={a.y - NH / 2} width={NW} height={NH} rx={10}
                fill={a.p.isRoot ? T.ledger : T.card} stroke={a.p.isRoot ? T.ledger : T.line} strokeWidth="1.5" />
              <text x={a.x} y={a.y - 3} textAnchor="middle" style={{ fontFamily: T.serif, fontSize: 14, fill: a.p.isRoot ? T.card : T.ink }}>
                {String(a.p.name).length > 15 ? String(a.p.name).slice(0, 14) + "\u2026" : String(a.p.name)}
              </text>
              <text x={a.x} y={a.y + 13} textAnchor="middle" style={{ fontFamily: T.sans, fontSize: 10.5, fill: a.p.isRoot ? T.brassSoft : T.faded }}>
                {a.p.isRoot ? "\u2605" : String(a.p.rel || "").slice(0, 18)}
              </text>
            </g>
          ))}
        </svg>
      ...
      <p style={{ fontFamily: T.sans, fontSize: 11.5, color: T.faded, margin: "8px 0 0" }}>
        {ft("Double bar = a marriage. The drop from its middle goes to the children's bar. Right angles only, as in a drop-line chart.")}
      </p>
      {dupes.length > 0 && (
        <p style={{ fontFamily: T.sans, fontSize: 12, color: T.berry, margin: "6px 0 0" }}>
          {ft("Folded away as likely duplicates: ") + dupes.map(d => d.name).join(", ") + ". " + ft("Open them in People to merge or re-label.")}
        </p>
      )}
    </div>
  );
