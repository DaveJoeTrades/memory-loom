...
function pickVoice(lang) {
  try {
    ...
    if (lang === "zh") {
      // Chinese still needs an explicit match: leaving it to the engine often yields an English voice.
      return vs.find(v => /zh|cmn/i.test(v.lang) && v.default) || vs.find(v => /zh|cmn/i.test(v.lang)) || null;
    }
    // "Automatic" means the system/browser default — never override the voice the person chose in their OS.
...
function browserSpeak(text, opts) {
  try {
    ...
    const go = () => { try { synth.speak(u); } catch (e) {} };
...
async function generateFamilyBible(lang, seedName) {
  const prompt = 'Invent a coherent, ordinary family for testing a life-story app. Storyteller: ' + (seedName || "the storyteller") + '.\n' +
...
async function generateSampleStories(n, lang, seedName, onProgress, bibleIn) {
  ...
  const bible = bibleIn || await generateFamilyBible(lang, seedName);
  ...
  for (let k = 0; k < n; k += PER) {
    ...
    const prompt = 'You are writing test material for a family life-story app: ' + count + ' more stories told aloud by ' + (seedName || "the storyteller") + '.\n' +
      'Tell them from ' + (seedName || "the storyteller") + '\u2019s own point of view \u2014 their memories, their vantage point on the shared family events below.\n' +
      ...
      'Write each story 320\u2013450 words: first person, spoken rhythm, digressions and asides, at least two named relatives from the family above, one named place, and at least one year that fits the timeline. ' +
...
function JournalFlow({ journal, mutateJournal, speaker, rec, say, tts, setTts, goHome, goFamily, lang, t, keepAudio }) {
  ...
  useEffect(() => {
    ...
    if (phase === "promptQ" && prompts[pIdx]) say((reaction ? reaction + " " : "") + pT(prompts[pIdx]));
    if (phase === "fuQ" && fuQ) say((reaction ? reaction + " " : "") + fuQ);
...
function TreeGraph({ graph, onPick }) {
  const rows = [
    ["grandparents", "Grandparents"], ["parents", "Parents"], ["self", "★"],
  ...
  for (const p of (graph.people || [])) {
    const gen = genOf(p.rel);
    if (gen === "unplaced") continue;
    const row = gen === "siblings" || gen === "spouse" ? "self" : gen === "extended" ? null : gen;
    if (!row) continue;
    (placed[row] = placed[row] || []).push(p);
  }
  (placed.self = placed.self || []);
  if (!placed.self.some(p => p.name === rootName)) placed.self.unshift({ id: "__root", name: rootName, rel: "self", isRoot: true });
  const W = 720, RH = 96, PAD = 16;
  const usedRows = rows.filter(r => (placed[r[0]] || []).length);
  const H = Math.max(140, usedRows.length * RH + PAD * 2);
  const nodeW = 118, nodeH = 42;
  const pos = {};
  usedRows.forEach((r, ri) => {
    const list = placed[r[0]] || [];
    const gap = W / (list.length + 1);
    list.forEach((p, i) => { pos[p.id] = { x: gap * (i + 1), y: PAD + ri * RH + nodeH / 2, p, row: r[0] }; });
  });
  const selfY = usedRows.findIndex(r => r[0] === "self");
  return (
    <div style={{ overflowX: "auto" }}>
      <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", minWidth: 520, height: H, display: "block" }}>
        {usedRows.map((r, ri) => (
          <text key={r[0]} x={6} y={PAD + ri * RH - 6} style={{ fontFamily: T.sans, fontSize: 10, fill: T.faded, letterSpacing: "0.1em", textTransform: "uppercase" }}>{ft(r[1])}</text>
        ))}
        {Object.values(pos).map(a => {
          // Draw a line from each person to the row above, so generations read as connected.
          const ri = usedRows.findIndex(r => r[0] === a.row);
          if (ri <= 0) return null;
          const parentRow = usedRows[ri - 1][0];
          const parents = Object.values(pos).filter(b => b.row === parentRow);
          if (!parents.length) return null;
          const mid = parents.reduce((sum, b) => sum + b.x, 0) / parents.length;
          return <path key={"e" + a.p.id} d={"M " + a.x + " " + (a.y - nodeH / 2) + " C " + a.x + " " + (a.y - RH / 2) + ", " + mid + " " + (a.y - RH / 2) + ", " + mid + " " + (a.y - RH + nodeH / 2)}
            fill="none" stroke={T.line} strokeWidth="1.5" />;
        })}
        {Object.values(pos).map(a => (
          <g key={a.p.id} onClick={() => onPick && a.p.id !== "__root" && onPick(a.p)} style={{ cursor: a.p.id === "__root" ? "default" : "pointer" }}>
            <rect x={a.x - nodeW / 2} y={a.y - nodeH / 2} width={nodeW} height={nodeH} rx={10}
              fill={a.p.isRoot ? T.ledger : T.card} stroke={a.p.isRoot ? T.ledger : T.line} strokeWidth="1.5" />
            <text x={a.x} y={a.y - 3} textAnchor="middle" style={{ fontFamily: T.serif, fontSize: 13.5, fill: a.p.isRoot ? T.card : T.ink }}>
              {String(a.p.name).slice(0, 14)}
            </text>
            <text x={a.x} y={a.y + 12} textAnchor="middle" style={{ fontFamily: T.sans, fontSize: 10, fill: a.p.isRoot ? T.brassSoft : T.faded }}>
              {a.p.isRoot ? "★" : String(a.p.rel || "").slice(0, 16)}
            </text>
          </g>
        ))}
      </svg>
...
function EntityCard({ e, kind, mutateGraph, index, onOpenStory, speakSummary }) {
  ...
  async function narrate() {
    if (narrating) { stopSpeak(); setNarrating(false); return; }
    setNarrating(true);
    try { await speakSummary(e, kind, storyIds); } finally { setNarrating(false); }
  }
            ...
            <div style={{ marginTop: 10 }}>
              ...
              {speakSummary && (
                <Btn small variant="brass" style={{ marginTop: 8 }} onClick={narrate}>
                  {narrating ? ft("Stop") : "▶ " + ft("Tell me about them")}
                </Btn>
              )}
            </div>
          )}
...
function FamilyView({ graph, mutateGraph, index, setIndexPersist, runExtraction, goStory, storageOk, journal, mutateJournal }) {
  ...
  const [seedN, setSeedN] = useState(10);
  ...
  async function seedGeneratedFor(spId, spName, count, bibleIn) {
    ...
    for (let i = 0; i < made.length; i++) {
      ...
      const rec2 = { id, question: made[i].q, chapter: made[i].chapter, bankId: null, inboxId: null, photoId: null, gentleId: null,
        ...
        startedAt: now - (made.length - i) * day, durMs: 150000, audioParts: 0, demo: true };
    ...
    setIndexPersist(ix => {
      ids.forEach((id, i) => {
        ...
        ix.meta[id] = { q: made[i].q.slice(0, 90), date: now - (made.length - i) * day, dur: 150000,
          sp: spName, extract: "pending", chapter: made[i].chapter, photoId: null, demo: true };
      });
    });
  ...
  async function seedGenerated() {
    ...
    try {
      ...
      const speakers = (graph.settings.speakers || []).slice(0, 4);
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        try {
          ...
          if (st && st.transcript) await runExtraction(st);
        } catch (e) {}
      }
  ...
  async function clearDemo() {
    ...
    const demoIds = index.storyIds.filter(id => (index.meta[id] || {}).demo);
  ...
  function openStoryFromEntity(sid) { setTab("stories"); setOpenStory(sid); loadStory(sid); }
  async function speakEntitySummary(e, kind, storyIds) {
    const bits = [];
    ...
    if (!bits.length) { speak(name + ". " + (e.details || []).join(". "), { lang: LOOM_LANG }); return; }
  ...
  async function importAudioFiles(files) {
    ...
    try {
      ...
      setAudioIn(r.ids + " " + ft("recordings re-linked") + (r.skipped ? " · " + r.skipped + " " + ft("skipped (name not recognised)") : ""));
  ...
  async function downloadAllAudio() {
    ...
    if (typeof window !== "undefined" && window.__zip) {
      ...
      try {
        ...
        await saveFilesSmart([new File([zip], "memory-loom-audio.zip", { type: "application/zip" })]);
        setAudioMsg(files.length + " " + ft("recordings zipped — choose Save to Files"));
        return;
            ...
            {pinForgot ? (
              <div style={{ marginTop: 16, padding: 12, borderRadius: 10, background: T.card, border: `1px solid ${T.line}` }}>
                <p style={{ fontFamily: T.sans, fontSize: 13, color: T.ink, margin: "0 0 10px", lineHeight: 1.5 }}>
                  {ft("Removing the PIN opens the ledger for anyone holding this device. Your stories are untouched.")}
                </p>
                <Btn small variant="danger" onClick={() => { mutateGraph(g => { g.settings.pin = ""; }); setPinOk(true); setPinForgot(false); }}>
                  {ft("Remove the PIN")}
                </Btn>
              </div>
            ) : (
          ...
          {tab === "journal" && (() => {
            ...
            return (
              <div style={{ maxWidth: 760 }}>
                ...
                {speakers.length === 0 && <Card><p style={{ fontFamily: T.sans, color: T.faded, margin: 0 }}>{ft("Journals appear once someone starts their daily few minutes on the storyteller screen.")}</p></Card>}
          ...
          {tab === "ask" && (
            <div style={{ maxWidth: 640 }}>
              <Card>
                ...
                <input value={fromName} onChange={e => setFromName(e.target.value)} placeholder={ft("Who's asking? (optional)")}
            ...
            <div style={{ maxWidth: 640 }}>
              ...
              <Card style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <span style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    <input type="number" min="1" max="100" value={seedN}
                      onChange={e => setSeedN(Math.max(1, Math.min(100, parseInt(e.target.value || "10", 10))))}
                  ...
                  <Btn small variant="ghost" onClick={clearDemo}>{ft("Remove sample")}</Btn>
              ...
              <Card style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginTop: 8 }}>
                  ...
                  {typeof window !== "undefined" && window.__audioGet ? <Btn small variant="ghost" onClick={downloadAllAudio}>{"⬇"} {ft("All audio files (zip)")}</Btn> : null}
                  {typeof window !== "undefined" && window.__importAudio ? (
                    <>
                      <input ref={audioInRef} type="file" accept="audio/*" multiple style={{ display: "none" }}
